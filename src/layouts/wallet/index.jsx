              ...
              <div className='px-3'>
                <Tab.List className='flex rounded-xl bg-violet-600 p-1'>
                  <Tab className={({ selected }) =>
                    classNames('w-full rounded-lg py-1 text-sm font-medium leading-5 text-blue-700',
                      selected
                            ...
                            <table className="w-full shadow-md text-sm text-left text-lightText dark:text-darkText">
                              ...
                              <tbody>
                                {bscTokenBalance.map((x) => (
                                  <tr onMouseEnter={() => setToken({ address: x.token_address, decimal: x.decimals, name: x.name, symbol: x.symbol, balance: Number(Number(x.balance) / 10 ** x.decimals) })} key={x.token_address} className="bg-white dark:bg-darkCard hover:bg-indigo-100 dark:hover:bg-zinc-800 w-full cursor-pointer">
                                    ...
                                    <td className='px-6 py-4'>
                                      <a href={`https://bscscan.com/address/${x.token_address}`} target='_blank' rel="noreferrer" className='text-lightText dark:text-darkText hover:underline'>{shortAddress(x.token_address, 5)}</a>
                                    </td>
                                    <td className='px-6 py-4 z-10 relative'>
                                      <p onClick={chainId === 56 ? () => setModalOpen(!modalOpen) : () => alert('გადართეთ BSC ქსელზე')} className='text-lightText dark:text-darkText hover:underline'>გაგზავნა</p>
                                    </td>
                                    <td className='px-6 py-4 z-10 relative'>
                              ...
                              <tbody>
                                {ethTokenBalance.map((x) => (
                                  <tr onMouseEnter={() => setToken({ address: x.token_address, decimal: x.decimals, name: x.name, symbol: x.symbol, balance: Number(Number(x.balance) / 10 ** x.decimals) })} key={x.token_address} className="bg-white dark:bg-darkCard hover:bg-indigo-100 dark:hover:bg-zinc-800 w-full cursor-pointer">
                                    ...
                                    <td className='px-6 py-4'>
                                      <a href={`https://etherscan.io/token/${x.token_address}`} target='_blank' rel="noreferrer" className='text-lightText dark:text-darkText hover:underline'>{shortAddress(x.token_address, 5)}</a>
                                    </td>
                                    <td className='px-6 py-4 z-10 relative'>
                                      <p onClick={chainId === 1 ? () => setModalOpen(!modalOpen) : () => alert('გადართეთ BSC ქსელზე')} className='text-lightText dark:text-darkText hover:underline'>გაგზავნა</p>
                                    </td>
                                    <td className='px-6 py-4 z-10 relative'>
                                      <AddToMetamask variant='text' address={x.token_address} decimal={x.decimals} symbol={x.symbol} />
                                    </td>
                              ...
                              <tbody>
                                {avaxTokenBalance.map((x) => (
                                  <tr onMouseEnter={() => setToken({ address: x.token_address, decimal: x.decimals, name: x.name, symbol: x.symbol, balance: Number(Number(x.balance) / 10 ** x.decimals) })} key={x.token_address} className="bg-white dark:bg-darkCard hover:bg-indigo-100 dark:hover:bg-zinc-800 w-full cursor-pointer">
                                    ...
                                    <td className='px-6 py-4'>
                                      <a href={`https://snowtrace.io/token/${x.token_address}`} target='_blank' rel="noreferrer" className='text-lightText dark:text-darkText hover:underline'>{shortAddress(x.token_address, 5)}</a>
                                    </td>
                                    <td className='px-6 py-4 z-10 relative'>
                                      <p onClick={chainId === 43114 ? () => setModalOpen(!modalOpen) : () => alert('გადართეთ AVAX ქსელზე')} className='text-lightText dark:text-darkText hover:underline'>გაგზავნა</p>
                                    </td>
                                    <td className='px-6 py-4 z-10 relative'>
                                      <AddToMetamask variant='text' address={x.token_address} decimal={x.decimals} symbol={x.symbol} />
                                    </td>
